---
course_id: IC5-L0
unit_id: IC5-L0-U07
content_status: verified
learner_status: no_evidence
---

# Unit 07 — Does it have a view?

## 来源与定位

- SB：印刷页44-49 / PDF50-55；来源`SRC-IC5-L0-SB`。
- TB：单元补充PDF76-77；教学T44-T49 / PDF78-83；Language Summary PDF210；WB答案PDF226；来源`SRC-IC5-L0-TB`。
- WB：印刷页37-42 / PDF40-45；来源`SRC-IC5-L0-WB`。
- Grammar Plus：SB印刷页138 / PDF144；TB PDF190。
- 官方课堂音频：前缀`IC5_L0_Unit 07`，10个MP3；来源`SRC-IC5-L0-AUDIO`。
- 与Unit 08合并检查：SB印刷页56-57 / PDF62-63；TB T56-T57 / PDF92-93；音频`IC5_L0_Unit 07-08 PC`（1个）。

## 单元学习结果

学习者应能描述房屋、公寓、房间和家具，询问住所是否有某设施，并说明某处有什么或没有什么。

## 核心语言包

- 主题词汇：房屋与公寓；房间；家具；设施与景观。
- 交际功能：描述住所；询问设施；说明房间内物品；比较理想住所的基本特征。
- 语法形式：一般现在时简答；`there is/there are`及否定；`some/any/no`。
- 意义与使用：用`there is/are`引入某地存在的事物；按单复数选择`is/are`；在肯定、疑问和否定环境中选择数量词。
- 发音：`th`音；听清房间、家具、数量与有无。
- 听力：识别住宅特点和购买家具的信息。
- 阅读与写作：阅读独特旅馆/住所简介；写理想住所的简短描述。
- 简短模型（改写）：`Is there a balcony?`；`There are two rooms.`；`It doesn't have a view.`
- TB提醒的易错点：`there is/are`单复数不一致；`some/any/no`使用混乱；把`have`和`there be`结构混成一句。

## 四次60分钟课程

| 课次 | 教材页 | 60分钟推进 | 当堂证据 |
|---|---|---|---|
| `IC5-L0-U07-D1` | SB印刷44-45 / PDF50-51 | 8分钟旧知提取；12分钟住所/房间；15分钟原版住宅音频首听；15分钟问答；10分钟无稿描述住处 | 首听设施结果；无稿描述；未知词 |
| `IC5-L0-U07-D2` | SB印刷46 / PDF52 | 8分钟提取；17分钟`there is/are`与数量词；10分钟控制练习；10分钟`th`听辨；10分钟替换问答；5分钟出口任务 | 首次答案与订正；3组有无句；发音记录 |
| `IC5-L0-U07-D3` | SB印刷47-48 / PDF53-54 | 8分钟提取；12分钟家具/设施输入；15分钟购物/住所细听；15分钟独特住所阅读；10分钟少提示比较 | 首听细节；阅读信息表；少提示口述 |
| `IC5-L0-U07-D4` | SB印刷49 / PDF55 | 10分钟间隔提取；15分钟末页综合；10分钟Grammar Plus U7；10分钟WB精选；10分钟官方U7口试；5分钟记录 | WB首次结果；理想住所短写；口试五维分；复习日期 |

## 测评位置

- 单元口试：`IC5_Intro_OQ_Question_Bank.pdf`的Unit 7分区；评分表`IC5_Intro_OQ_Scoring_Sheet.pdf`。
- 双单元书面测验：Unit 08后使用`Intro Written_Quiz_U7-8/IC5_Intro_WQ_U7to8.pdf`及同文件夹MP3 Track04；本单元结束时仅登记待测。

## 个性化工作场景迁移（非教材原文）

模拟为出差选择住宿：询问房间、网络、书桌和景观等设施，再说明哪一处更适合工作。该任务是个性化扩展，不是教材原文。

## 必须保存的学习证据

- 原版音频首听中的住所/家具信息。
- 一段无稿住所描述和一轮设施问答；一篇理想住宿短写。
- 控制练习和WB首次/订正结果。
- 官方U7口试原始分；Unit 08后补充U7-8测验分项。

## 验收门槛（仓库规则，非出版社官方线）

- `covered`：四课均有真实记录，且听、说、读、写和首次练习证据齐全。
- `meets_standard`：首次代表性任务≥80%；口试≥18/25且各维≥3；U7-8测验关键目标≥80%。
- `can_use`：在新的住宿选择场景中无内容提示询问并描述设施，`there is/are`与数量词至少80%准确，最多一次澄清。
- `retained`：D+2和D+7换住所布局与设施复测仍达标。

当前没有该单元真实学习证据，`learner_status`保持`no_evidence`。
