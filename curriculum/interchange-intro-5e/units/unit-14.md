---
course_id: IC5-L0
unit_id: IC5-L0-U14
content_status: verified
learner_status: no_evidence
---

# Unit 14 — I had a good time.

## 来源与定位

- SB：印刷页92-97 / PDF98-103；来源`SRC-IC5-L0-SB`。
- TB：单元补充PDF138-139；教学T92-T97 / PDF140-145；Language Summary PDF217；WB答案PDF233；来源`SRC-IC5-L0-TB`。
- WB：印刷页79-84 / PDF82-87；来源`SRC-IC5-L0-WB`。
- Grammar Plus：SB印刷页145 / PDF151；TB PDF197。
- 官方课堂音频：前缀`IC5_L0_Unit 14`，12个MP3；来源`SRC-IC5-L0-AUDIO`。
- U13-14检查：SB印刷页98-99 / PDF104-105；TB T98-T99 / PDF146-147；音频`IC5_L0_Unit 13-14 PC`（1个）。

## 单元学习结果

学习者应能谈论上个周末、家务、假期和已经发生的活动，询问对方是否做过某事，并给出肯定或否定的简短回答。

## 核心语言包

- 主题词汇：周末活动；家务；假期活动；过去时间表达。
- 交际功能：叙述过去活动；询问过去是否做过某事；回应并补充过去经历。
- 语法形式：一般过去时规则/不规则动词；一般疑问和简答；`didn't + 动词原形`。
- 意义与使用：用过去式标记已结束的事件；用`did`承担问句/否定的过去标记，主要动词还原。
- 发音：规则过去式`-ed`的不同读音；不规则过去式的清晰度。
- 听力：识别暑期/周末活动及过去发生的信息。
- 阅读与写作：阅读周末帖子；写上个周末的简短记录。
- 简短模型（改写）：`I visited friends.`；`Did you work?`；`No, I didn't.`
- TB提醒的易错点：不规则动词套用`-ed`；`did/didn't`后仍用过去式；`-ed`一律读成额外音节。

## 四次60分钟课程

| 课次 | 教材页 | 60分钟推进 | 当堂证据 |
|---|---|---|---|
| `IC5-L0-U14-D1` | SB印刷92-93 / PDF98-99 | 8分钟U13提取；12分钟周末/家务；15分钟原版过去活动首听；15分钟问答；10分钟无稿说上周末 | 首听活动结果；无稿过去叙述；未知词 |
| `IC5-L0-U14-D2` | SB印刷94 / PDF100 | 8分钟提取；17分钟规则/不规则过去式与`did`；10分钟控制练习；10分钟`-ed`听辨；10分钟替换问答；5分钟出口任务 | 首次答案与订正；3组过去问答；发音记录 |
| `IC5-L0-U14-D3` | SB印刷95-96 / PDF101-102 | 8分钟提取；12分钟假期活动；15分钟过去事件细听；15分钟周末帖子阅读；10分钟少提示互访 | 首听细节；阅读信息表；互访问答转写 |
| `IC5-L0-U14-D4` | SB印刷97 / PDF103 | 10分钟间隔提取；15分钟末页综合；10分钟Grammar Plus U14；10分钟WB精选；10分钟官方U14口试；5分钟记录 | WB首次结果；周末短写；口试五维分；复习日期 |

## 测评位置

- 单元口试：`IC5_Intro_OQ_Question_Bank.pdf`的Unit 14分区；使用`IC5_Intro_OQ_Scoring_Sheet.pdf`。
- 双单元检查：SB U13-14 Progress Check及`IC5_L0_Unit 13-14 PC`。
- 双单元书面测验：`Intro Written_Quiz_U13-14/IC5_Intro_WQ_U13to14.pdf`与同文件夹MP3 Track07。

## 个性化工作场景迁移（非教材原文）

模拟简短工作回顾：说明昨天完成了什么、没完成什么，询问同事是否做了某项任务，并补充一个结果。该任务是个性化扩展，不是教材原文。

## 必须保存的学习证据

- 原版音频首听的过去活动结果。
- 一段无稿周末/工作回顾和一轮过去事件问答；一篇过去活动短写。
- 控制练习、WB、U13-14 Progress Check和Written Quiz首次/订正结果。
- Unit 14官方口试五维原始分。

## 验收门槛（仓库规则，非出版社官方线）

- `covered`：D1-D4均有真实日期/时长及听说读写证据。
- `meets_standard`：首次代表性练习与U13-14关键测验目标≥80%；口试≥18/25且各维≥3。
- `can_use`：在新工作回顾情境中无内容提示叙述并询问过去事件，规则/不规则过去式和`did + 原形`至少80%准确，最多一次澄清。
- `retained`：D+2和D+7更换过去时间与活动复测仍达标。

当前没有该单元真实学习证据，`learner_status`保持`no_evidence`。
