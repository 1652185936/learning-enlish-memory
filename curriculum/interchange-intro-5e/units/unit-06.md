---
course_id: IC5-L0
unit_id: IC5-L0-U06
content_status: verified
learner_status: no_evidence
---

# Unit 06 — I ride my bike to school.

## 来源与定位

- SB：印刷页36-41 / PDF42-47；来源`SRC-IC5-L0-SB`。
- TB：单元补充PDF66-67；教学T36-T41 / PDF68-73；Language Summary PDF209；WB答案PDF225；来源`SRC-IC5-L0-TB`。
- WB：印刷页31-36 / PDF34-39；来源`SRC-IC5-L0-WB`。
- Grammar Plus：SB印刷页137 / PDF143；TB PDF189。
- 官方课堂音频：前缀`IC5_L0_Unit 06`，10个MP3；来源`SRC-IC5-L0-AUDIO`。
- U5-6检查：SB印刷页42-43 / PDF48-49；TB T42-T43 / PDF74-75；音频`IC5_L0_Unit 05-06 PC`（1个）。

## 单元学习结果

学习者应能谈论上学/上班交通方式、家人和每周日常安排，询问并回答他人何时或如何做日常活动。

## 核心语言包

- 主题词汇：交通方式；家庭成员；日常活动；星期；时间表。
- 交际功能：描述通勤和例行活动；询问他人的日程；说明一周中何时做某事。
- 语法形式：一般现在时规则/不规则动词陈述；一般疑问、简答和Wh问句；时间表达。
- 意义与使用：一般现在时表达习惯和固定安排；第三人称单数形式与主语对应；`does`问句后使用动词原形。
- 发音：第三人称单数`-s`词尾；听清星期、时间和交通方式。
- 听力：识别人物的日常活动、交通方式和日期/星期。
- 阅读与写作：阅读日常时间表；写个人一周例行活动。
- 简短模型（改写）：`I take the bus.`；`Does she work on Monday?`；`When does he leave?`
- TB提醒的易错点：第三人称漏`-s`；`does`后仍加`-s`；把习惯错误地写成现在进行时。

## 四次60分钟课程

| 课次 | 教材页 | 60分钟推进 | 当堂证据 |
|---|---|---|---|
| `IC5-L0-U06-D1` | SB印刷36-37 / PDF42-43 | 8分钟U5提取；12分钟交通/家庭；15分钟原版日常音频首听；15分钟通勤问答；10分钟无稿说自己的路线 | 首听交通结果；无稿通勤描述；未知词 |
| `IC5-L0-U06-D2` | SB印刷38 / PDF44 | 8分钟提取；17分钟一般现在时和第三人称；10分钟控制练习；10分钟`-s`发音；10分钟替换问答；5分钟出口任务 | 首次答案与订正；3组第三人称句；发音记录 |
| `IC5-L0-U06-D3` | SB印刷39-40 / PDF45-46 | 8分钟提取；12分钟星期/例行活动；15分钟日程细听；15分钟时间表阅读与填写；10分钟少提示采访 | 首听日程结果；一周时间表；采访转写 |
| `IC5-L0-U06-D4` | SB印刷41 / PDF47 | 10分钟间隔提取；15分钟末页综合；10分钟Grammar Plus U6；10分钟WB精选；10分钟官方U6口试；5分钟记录 | WB首次结果；例行活动短写；口试五维分；复习日期 |

## 测评位置

- 单元口试：`IC5_Intro_OQ_Question_Bank.pdf`的Unit 6分区；使用`IC5_Intro_OQ_Scoring_Sheet.pdf`。
- 双单元检查：SB U5-6 Progress Check及`IC5_L0_Unit 05-06 PC`。
- 双单元书面测验：`Intro Written_Quiz_U5-6/IC5_Intro_WQ_U5to6.pdf`与同文件夹MP3 Track03；首次作答后核对答案。

## 个性化工作场景迁移（非教材原文）

模拟向同事说明自己的通勤方式、工作日时间表和团队成员的固定安排，再询问一次例会何时举行。该任务是个性化扩展，不是教材原文。

## 必须保存的学习证据

- 原版音频首听的交通、星期和活动结果。
- 无稿通勤描述与一轮日程采访；一份每周例行活动短写。
- 控制练习、WB、U5-6 Progress Check和Written Quiz首次/订正结果。
- Unit 6官方口试五维原始分。

## 验收门槛（仓库规则，非出版社官方线）

- `covered`：D1-D4均有真实日期/时长及完整听说读写证据。
- `meets_standard`：首次代表性练习与U5-6关键测验目标≥80%；口试≥18/25且各维≥3。
- `can_use`：在新工作日程场景中无内容提示描述自己和第三人的固定安排，第三人称及`does + 原形`至少80%准确，最多一次澄清。
- `retained`：D+2和D+7换人物、星期与交通方式复测仍达标。

当前没有该单元真实学习证据，`learner_status`保持`no_evidence`。
