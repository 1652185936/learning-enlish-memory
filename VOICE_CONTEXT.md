...
- 最近更新：2026-09-05
- 作用：每次新语音对话首先读取本文件，再读取当前课次需要的少量文件。
...
- 主要用途：工作英语。
- 长期方向：CEFR C2。
- 考试方向：雅思；类别、总分、单项和日期待确认。
- 课程起点：Interchange Fifth Edition Intro。
- 当前真实水平：待基线评估；选择Intro不等于已经判定为A1。
...
- 课程：`IC5-L0` Interchange Fifth Edition Intro。
- 课程状态：`planned_not_started`。
- 下一课：`IC5-L0-U01-D1`。
- 计划日期：2026-09-07；若未实际开始，顺延而不是补写完成记录。
- 教材：Unit 1 *What’s your name?*，学生书印刷页2-3。
- 本课目标：问候、姓名、自我介绍与`be`的第一轮输入；不是只练一句目标句。
- 当前周计划：[2026-W37](plans/weekly/2026-W37.md)。
...
> 上次真实进度为0；当前没有到期复习；今天计划开始IC5-L0-U01-D1，学习Unit 1印刷页2-3。

然后直接进入完整课程。教师每次指令要短，但必须继续提取、输入、控制练习、互动、独立输出和出口检查，不得在两句示例后结束。
...
若当前模式不能写仓库，明确说“尚未保存”，并输出结构化交接记录。
